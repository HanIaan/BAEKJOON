...
int main() {
    ...
    vector<pair<string,int>> wordVec(wordMap.begin(), wordMap.end());   //map -> vector

    sort(wordVec.begin(), wordVec.end(), [](auto &a, auto &b) {
        if(a.second != b.second) return a.second > b.second;        // 1순위 빈도
        if (a.first.size() != b.first.size()) return a.first.size() > b.first.size();   // 2순위 길이
